...
async (dispatch, getState) => {
  ...
  try {
    const res = await axios.get(`https://jsonplaceholder.typicode.com/posts?userId=${usuario_id}`);

    const nuevas = res.data.map((publicacion) => ({ 
      ...publicacion, 
      comentarios: [], 
      abierto: false 
    }));
    ...
    dispatch({ type: ERROR, payload: 'Publicaciones no disponibles' });
  }
};
...
export const traerComentarios = (pub_key, com_key) => 
  ...
  dispatch({ type: COM_CARGANDO })

  const { publicaciones } = getState().publicacionesReducer;
  const seleccionada = publicaciones[pub_key][com_key];

 try {
  const res = await axios.get(`https://jsonplaceholder.typicode.com/comments?postId=${seleccionada.id}`);
